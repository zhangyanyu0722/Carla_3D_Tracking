...
#         __sets[name] = (lambda split=split, version=version: vg(version, 
#         split))
for version in ['150-50-20', '150-50-50', '500-150-80', '750-250-150',
                '1750-700-450', '1600-400-20']:
    for split in ['minitrain', 'smalltrain', 'train', 'minival', 'smallval',
                  'val', 'test']:
        name = 'vg_{}_{}'.format(version, split)
...
CARLA_DATA_PATH = "data/carla"
for split in ["train", "val", "test"]:
    name = "carla_{}".format(split)

    # Lambda parameter_list : expression, returns a function name
    __sets[name] = (lambda 
                    s = split
                    : Carla(CARLA_DATA_PATH, s))
