...
if __name__ == '__main__':
    ...
    if args.dataset == "pascal_voc":
        ...
        # cfg.VEHICLE_CLASSIFICATION = False # possible extensions on the classification of vehicles
        cfg.USE_DEBUG_SET = False
        cfg.BINARY_CLASS = True
        cfg.ANNO_PATH = 'train'
    ...
    fixed_center = Variable(fixed_center)
    ...
    for key, value in dict(fasterRCNN.named_parameters()).items():
        if value.requires_grad:
            if 'bias' in key:
                params += [
                    {'params': [value], 'lr': lr * (cfg.TRAIN.DOUBLE_BIAS +
                                                    1), \
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            RCNN_loss_center, rois_label = fasterRCNN(im_data, im_info,
                                                      gt_boxes, num_boxes,
            ...
            if step % args.disp_interval == 0:
                ...
                if args.mGPUs:
                    ...
                    loss_rcnn_center = RCNN_loss_center.mean().item()
                    ...
                    loss_rcnn_center = RCNN_loss_center.item()
                    fg_cnt = torch.sum(rois_label.data.ne(0))
                    bg_cnt = rois_label.data.numel() - fg_cnt

                print(
                    "[session %d][epoch %2d][iter %4d/%4d] loss: %.4f, "
                    "lr: %.2e" \
                    % (
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, "
                    "rcnn_box %.4f, rcnn_center %.4f" \
                ...
                if args.use_tfboard:
                    info = {
                        ...
                        'loss_rcnn_center': loss_rcnn_center
                    }
        ...
        save_checkpoint({
            ...
            'epoch': epoch + 1,
            'model': fasterRCNN.module.state_dict() if args.mGPUs else
