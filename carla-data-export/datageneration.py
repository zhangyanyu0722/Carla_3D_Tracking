...
folders = ['calib', 'image', 'label', 'velodyne', 'planes']
...
for folder in folders:
    directory = os.path.join(OUTPUT_FOLDER, folder, TIME_ON_NEW_EPISODE)
    maybe_create_dir(directory)

""" DATA SAVE PATHS """
GROUNDPLANE_PATH = os.path.join(OUTPUT_FOLDER, "planes", TIME_ON_NEW_EPISODE, '{0:06}.txt')
LIDAR_PATH = os.path.join(OUTPUT_FOLDER, "velodyne", TIME_ON_NEW_EPISODE, '{0:06}.bin')
LABEL_PATH = os.path.join(OUTPUT_FOLDER, "label", TIME_ON_NEW_EPISODE, '{0:06}.json')
IMAGE_PATH = os.path.join(OUTPUT_FOLDER, "image", TIME_ON_NEW_EPISODE, '{0:06}.jpg')
CALIBRATION_PATH = os.path.join(OUTPUT_FOLDER, "calib", TIME_ON_NEW_EPISODE, '{0:06}.txt')
START_NUM = 10000
...
class CarlaGame(object):
    ...
    def _generate_datapoints(self, image):
        ...
        datapoints['name']=imgNameFmt.format(TIME_ON_NEW_EPISODE, self.captured_frame_no)
        ...
        datapoints['attributes'] = {"weather": "clouds", "scene": "undefined", "timeofday": "noon"}
        datapoints['intrinsics'] = {"focal": [960.0000000000001, 960.0000000000001], 
                                    "center": [960, 540], 
                                    "fov": 90.0, 
                                    "nearClip": 0.15,
                                    "cali": [[960.0000000000001, 0, 960, 0], 
                                             [0, 960.0000000000001, 540, 0], 
                                             [0, 0, 1, 0]]}
        ...
        datapoints['extrinsics'] = {"location": [-loc.x, -loc.y, loc.z],
                                   "rotation": [degrees_to_radians(rot.pitch),
        ...
        for agent in self._measurements.non_player_agents:
            if should_detect_class(agent) and GEN_DATA:
                ...
                if kitti_datapoint:
                    ...
                    orientation = degrees_to_radians(agent.vehicle.transform.rotation.yaw)

                    ob_center_x = (kitti_datapoint.bbox[2] + kitti_datapoint.bbox[0])/2

                    rotation_y2alpha = orientation - np.arctan2(960.0000000000001, (ob_center_x-960))
    ...
    def _save_training_files(self, datapoints, point_cloud):
        ...
        save_ref_files(OUTPUT_FOLDER, TIME_ON_NEW_EPISODE, PHASE, self.captured_frame_no)
        save_image_data(
